...
class VectorStore:
    ...
    def search(
        ...
        try:
            ...
            error_msg = str(e)
            if (
                "No embeddings found" in error_msg
                or "empty collection" in error_msg.lower()
            ):
                return SearchResults.empty(
                    "No course content available. Please load course documents first."
                )
            elif "metadata" in error_msg.lower():
                return SearchResults.empty(
                    "Course data format error. Please reload course documents."
                )
            else:
